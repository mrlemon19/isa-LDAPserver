...
searchTree::searchTree(std::vector<unsigned char> filter,
                       std::vector<unsigned char> attributes, int messageID,
                       int clientSocket, std::string DBfileName,
                       packetSender *ps, int sizeLimit) {

  this->root = new searchNode(filter);
  this->attributes = parseAttributes(attributes);
  this->messageID = messageID;
  this->clientSocket = clientSocket;
  this->maxResEnt = sizeLimit;
...
void searchTree::search() {
  ...
  while (std::getline(DBStream, line)) {
    ...
    if (this->root->evaluate(&entry) && resEntSend < this->maxResEnt) {
      ...
      if (std::find(this->attributes.begin(), this->attributes.end(), MAIL) !=
          this->attributes.end()) {
      ...
      if (std::find(this->attributes.begin(), this->attributes.end(), CN) !=
          this->attributes.end()) {
      ...
      if (std::find(this->attributes.begin(), this->attributes.end(), UID) !=
          this->attributes.end()) {
...
std::vector<attributeType_t>
searchTree::parseAttributes(std::vector<unsigned char> attributes) {
  // parse attributes
...
void searchNode::parseFilter() {
  ...
  case 0xa0: {
    ...
    // sepatares filters from inside AND filter and creates searchNodes from
    // them
...
std::vector<std::vector<unsigned char>> searchNode::separateFilter() {
  ...
  while (this->filterIndex < this->filterLenght) {
    ...
    if (filterType == 0xa0 || filterType == 0xa1 || filterType == 0xa2 ||
        filterType == 0xa3 || filterType == 0xa4) {
      // filter starts
      int lenght = this->readChar();
...
ResultEntry::ResultEntry(std::string entry) {
  ...
  this->uid =
      entry.substr(firstSemicolon + 1, secondSemicolon - firstSemicolon - 1);
